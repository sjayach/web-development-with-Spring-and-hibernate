...
<ol class="breadcrumb">
	...
	<li>
		...
		<sec:authorize access="hasRole('ROLE_APPLICANT')">
			<a href="<c:url value='/applicant.html'/>">Applicant</a>
		</sec:authorize>
	</li>
	<sec:authorize access="hasAnyRole('ROLE_REVIEWER','ROLE_ADMIN')">
	<li>
		<a href="<c:url value='/viewApplications.html?jobId=${app.job.id}'/>">View Applications</a>
	</li>
	</sec:authorize>
	<li class="active">
		Application View
...
<table class="table table-striped">
	...
	<c:if test="${not empty app.cv }">
		...
		<a href="download.html?type=cs&fileId=${app.cv.id}"> Download</a>
		<br />
		<br />
	</c:if>
	
	<c:if test="${not empty app.researchStatement}">
		<b>Research Statement</b>
		<a href="download.html?type=ts&fileId=${app.researchStatement.id}">Download</a>
		<br />
		<br />
	</c:if>
	
	<c:if test="${not empty app.teachingStatement}">
		<b>Teaching Statement</b>
	 	<a href="download.html?type=rs&fileId=${app.teachingStatement.id }">Download</a>
	 	<br />
		<br />
	</c:if>
</div>
