...
<table class="table table-striped">
	...
	<c:forEach items="${appliedjobs}" var="job">
		<tr>
			<th>${job.title}</th>
			<td align="left"><span style="margin-left:300px;float:left"><a href="showApplication.html?id=${job.id }&userEmail=${appEmail}" class="btn btn-primary btn-sm active">Show Application</a></span>
		</tr>
	</c:forEach>
</table>
</fieldset>
		...
		<tr>
			...
			<c:forEach items="${availablejobs}" var="apply">
				<tr>
					...
					<td align="left"><span style="margin-left:300px;float:left"><a href="application.html?jobid=${apply.id}&appEmail=${appEmail}" class="btn btn-success btn-sm active">Apply</a></span></td>
				</tr>
			</c:forEach>
